...
class Geschaeftsplanung extends Page implements HasActions, HasForms
{
    ...
    public function save(): void
    {
        ...
        $plan->update([
            ...
            // Eingabe-Maske (Stufe 3)
            'plan_start_month' => (int) ($this->stamm['plan_start_month'] ?? 1) ?: 1,
            'neugruendung' => ! empty($this->stamm['neugruendung']),
            'mineraloel' => $this->stamm['mineraloel'] ?: null,
            'inhaber' => $this->stamm['inhaber'] ?: null,
            'bundesland' => $this->stamm['bundesland'] ?: null,
            'telefon' => $this->stamm['telefon'] ?: null,
            'email' => $this->stamm['email'] ?: null,
            'unternehmensform' => $this->stamm['unternehmensform'] ?: null,
            'mehrfachbetreiber' => $this->stamm['mehrfachbetreiber'] ?: null,
            'backshop' => ! empty($this->stamm['backshop']),
            'gastronomie' => ! empty($this->stamm['gastronomie']),
            'verderb_backshop_pct' => $this->num($this->stamm['verderb_backshop_pct'] ?? 0),
            'verderb_gastro_pct' => $this->num($this->stamm['verderb_gastro_pct'] ?? 0),
            'pfandschlupf' => ! empty($this->stamm['pfandschlupf']),
            'kaffeeautomat' => ! empty($this->stamm['kaffeeautomat']),
            'kaffeekonzept' => $this->stamm['kaffeekonzept'] ?: null,
            'anzahl_terminal' => (int) $this->num($this->stamm['anzahl_terminal'] ?? 0),
            'mautstation' => ! empty($this->stamm['mautstation']),
            'nebengeschaefte' => ! empty($this->stamm['nebengeschaefte']),
            'nebengeschaeft1' => $this->stamm['nebengeschaeft1'] ?: null,
            'nebengeschaeft2' => $this->stamm['nebengeschaeft2'] ?: null,
            'unternehmer_pkw' => ! empty($this->stamm['unternehmer_pkw']),
            'bruttolistenpreis' => $this->num($this->stamm['bruttolistenpreis'] ?? 0),
            'digitale_buchhaltung' => ! empty($this->stamm['digitale_buchhaltung']),
            'mandant_contax' => ! empty($this->stamm['mandant_contax']),
            'verfahrensdoku' => ! empty($this->stamm['verfahrensdoku']),
            'werkstatt' => ! empty($this->stamm['werkstatt']),
            'muenzgeraete' => ! empty($this->stamm['muenzgeraete']),
            'waschanlage' => ! empty($this->stamm['waschanlage']),
            'wasseraufbereitung' => ! empty($this->stamm['wasseraufbereitung']),
            'anzahl_waeschen' => (int) $this->num($this->stamm['anzahl_waeschen'] ?? 0),
            'waschpreis' => $this->num($this->stamm['waschpreis'] ?? 0),
            'anzahl_waeschen_1' => (int) $this->num($this->stamm['anzahl_waeschen_1'] ?? 0),
            'waschpreis_1' => $this->num($this->stamm['waschpreis_1'] ?? 0),
            'anzahl_waeschen_2' => (int) $this->num($this->stamm['anzahl_waeschen_2'] ?? 0),
            'waschpreis_2' => $this->num($this->stamm['waschpreis_2'] ?? 0),
    ...
    private function loadPlan(): void
    {
        ...
        $this->stamm = [
            ...
            // Eingabe-Maske (Stufe 3)
            'plan_start_month' => (string) ($plan->plan_start_month ?: 1),
            'neugruendung' => (bool) $plan->neugruendung,
            'mineraloel' => $plan->mineraloel,
            'inhaber' => $plan->inhaber,
            'bundesland' => $plan->bundesland,
            'telefon' => $plan->telefon,
            'email' => $plan->email,
            'unternehmensform' => $plan->unternehmensform,
            'mehrfachbetreiber' => $plan->mehrfachbetreiber,
            'backshop' => (bool) $plan->backshop,
            'gastronomie' => (bool) $plan->gastronomie,
            'verderb_backshop_pct' => $this->fmt($plan->verderb_backshop_pct),
            'verderb_gastro_pct' => $this->fmt($plan->verderb_gastro_pct),
            'pfandschlupf' => (bool) $plan->pfandschlupf,
            'kaffeeautomat' => (bool) $plan->kaffeeautomat,
            'kaffeekonzept' => $plan->kaffeekonzept,
            'anzahl_terminal' => (string) ($plan->anzahl_terminal ?: ''),
            'mautstation' => (bool) $plan->mautstation,
            'nebengeschaefte' => (bool) $plan->nebengeschaefte,
            'nebengeschaeft1' => $plan->nebengeschaeft1,
            'nebengeschaeft2' => $plan->nebengeschaeft2,
            'unternehmer_pkw' => (bool) $plan->unternehmer_pkw,
            'bruttolistenpreis' => $this->fmt($plan->bruttolistenpreis),
            'digitale_buchhaltung' => (bool) $plan->digitale_buchhaltung,
            'mandant_contax' => (bool) $plan->mandant_contax,
            'verfahrensdoku' => (bool) $plan->verfahrensdoku,
            'werkstatt' => (bool) $plan->werkstatt,
            'muenzgeraete' => (bool) $plan->muenzgeraete,
            'waschanlage' => (bool) $plan->waschanlage,
            'wasseraufbereitung' => (bool) $plan->wasseraufbereitung,
            'anzahl_waeschen' => (string) ($plan->anzahl_waeschen ?: ''),
            'waschpreis' => $this->fmt($plan->waschpreis),
            'anzahl_waeschen_1' => (string) ($plan->anzahl_waeschen_1 ?: ''),
            'waschpreis_1' => $this->fmt($plan->waschpreis_1),
            'anzahl_waeschen_2' => (string) ($plan->anzahl_waeschen_2 ?: ''),
            'waschpreis_2' => $this->fmt($plan->waschpreis_2),
